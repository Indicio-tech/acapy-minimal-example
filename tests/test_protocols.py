...
from tests.conftest import getenv_or_raise, alice, bob
...
@pytest.mark.asyncio
async def test_multiuse_didexchange(alice, bob):
    """Testing that dids are exchanged successfully with multiuse on."""
    alice_conn, bob_conn = await didexchange(alice, bob, multiuse=True)
    assert alice_conn
    assert bob_conn
    yield alice_conn, bob_conn
